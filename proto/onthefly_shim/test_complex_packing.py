"""Byte-level validation of the complex-coefficient packing layout.
...
would require PETSc compiled with complex scalars (not available in this
env). It verifies:
...
def _emulate_kernel_read(coeffs, entity_row, col_in_row, real_dtype,
    ...
    # Real-view position of the row's start.
...
def _pack_byte_layout(coeffs_dtype, real_dtype, n_cols_complex, n_extra_cols,
    ...
    ``entities[i]``: target column index of i-th payload cell.
    ``payloads[i]``: real-dtype 1D arrays (n_pts header + values).
    Returns (coeffs, expected_offsets_real_units).
    """
    ...
    # Write the relative offsets into the trailing offset column(s).
    ...
    # Write payloads through the real-view (matches custom_coefficients._copy_vals).
...
def check_one(coeffs_dtype, real_dtype):
    n_cols_complex = 5      # arbitrary genuine-coeff width
    ...
    # Now play kernel: for each entity, read offset from coeffs_offset
    # (last n_extra_cols complex cells of the entity's row -> first
    # complex cell of that block), then read len(payload) real slots.
    ...
    for i, p in enumerate(payloads):
        ...
        assert abs_pos == expected_abs[i], (
            f"{coeffs_dtype.name}: offset mismatch, "
            f"got abs={abs_pos} expected {expected_abs[i]}"
        )
        assert np.allclose(got, p), (
            f"{coeffs_dtype.name}: payload bytes mismatch at entity {i}"
        )
    print(f"  {coeffs_dtype.name:>11s} (real={real_dtype.name:>7s}): OK")


def main() -> int:
    check_one(np.dtype(np.float32), np.dtype(np.float32))
    check_one(np.dtype(np.float64), np.dtype(np.float64))
    check_one(np.dtype(np.complex64), np.dtype(np.float32))
    check_one(np.dtype(np.complex128), np.dtype(np.float64))
    print("RESULT: all coeffs dtypes OK")
    return 0
