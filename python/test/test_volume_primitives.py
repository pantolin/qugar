...
def compute_volume(
    ...
    one = dolfinx.fem.Constant(cart_mesh, dtype(1.0))
    ...
    dx = ufl.dx(domain=cart_mesh, subdomain_data=cell_subdomain_data, subdomain_id=full_tag)
...
def compute_boundary_area(
    ...
    one = dolfinx.fem.Constant(domain.cart_mesh, dtype(1.0))

    quad_degree = get_Gauss_quad_degree(n_quad_pts)
    ds = dx_bdry_unf(
        domain=domain.cart_mesh,
        subdomain_data=cell_subdomain_data,
