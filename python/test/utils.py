...
def get_dolfinx_forms(ufl_form) -> tuple[CustomForm, Form]:
...
def run_scalar_check(
    ...
    standard DOLFINx `Form` and the custom `CustomForm` feeded with the
    quadratures generated by `quad_gen`, and checks that both results
    ...
    custom_form, form = get_dolfinx_forms(ufl_form)
...
def run_vector_check(ufl_form, quad_gen, rtol=None, atol=None):
...
def run_matrix_check(ufl_form, quad_gen, rtol=None, atol=None):
