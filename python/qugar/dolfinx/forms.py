...
def form_custom(
    ...
    def _form(form) -> CustomForm:
        ...
        # must be modified: for unfitted meshes "everywhere" needs to
        # include the unfitted boundary, not just the mesh's external
        # facets. ``_modify_everywhere_exterior_facet_integrals``
        # replaces the ``"everywhere"`` subdomain id with a fresh
        # numerical one and tags the integral with a metadata flag so
        # the unfitted facets can be attached later.
        ...
        # Build the per-integral-type subdomain id list. DOLFINx 0.10.0
        # reads these directly out of ``ufcx_form.form_integral_ids``,
        # but qugar must reconstruct them from the UFL integrals: the
        # synthetic subdomain ids generated by
        # ``_modify_everywhere_exterior_facet_integrals`` for unfitted
        # everywhere-boundary integrals only exist on the UFL form,
        # and the unfitted-facet ranges appended below to
        # ``subdomains[IntegralType.exterior_facet]`` complete the
        # picture.
