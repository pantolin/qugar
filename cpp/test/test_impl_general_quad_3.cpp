...
TEST_CASE("General function quadrature for annulus (2D)", "[impl]")
{
  ...
  const auto annulus = std::make_shared<funcs::Annulus>(R0, R1, origin);
...
// NOLINTNEXTLINE(misc-use-anonymous-namespace,readability-function-cognitive-complexity)
TEST_CASE("General function quadrature for torus (3D)", "[impl]")
{
  using namespace qugar;
  using namespace qugar::impl;
  ...
  const auto torus = std::make_shared<funcs::Torus>(major_radius, minor_radius, origin, axis);
