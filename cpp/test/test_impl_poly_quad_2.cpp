...
//! @brief Test 2 for Implicit general Bezier polynomial unfitted implicit domain.
//! @version 0.0.2
//! @date 2025-01-21
//!
...
TEST_CASE("Quadrature for Bezier function for sphere 2D", "[impl]")
{
  ...
  const auto sphere = std::make_shared<funcs::SphereBzr<2>>(radius, origin);
...
// // NOLINTNEXTLINE(misc-use-anonymous-namespace,readability-function-cognitive-complexity)
TEST_CASE("Quadrature for Bezier function for sphere 3D", "[impl]")
{
  using namespace qugar;
  using namespace qugar::impl;
  ...
  const real radius{ 0.4 };
  const auto sphere = std::make_shared<funcs::SphereBzr<3>>(radius, origin);
