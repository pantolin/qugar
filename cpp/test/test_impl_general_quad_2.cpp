...
//! @file test_impl_impl_general_quad_2.cpp
//! @author Pablo Antolin (pablo.antolin@epfl.ch)
//! @brief Test 2 for Implicit general quadrature.
//! @version 0.0.1
//! @date 2025-01-21
//!
...
TEST_CASE("General function quadrature for plane 2D", "[impl]")
{
  using namespace qugar;
  using namespace qugar::impl;

  const Point<2> origin{ 0.5, 0.5 };
  const auto plane = std::make_shared<funcs::Plane<2>>(origin, Point<2>{ 2.0, 1.0 });

  const auto grid = std::make_shared<CartGridTP<2>>(std::array<std::size_t, 2>({ { 2, 2 } }));
...
// NOLINTNEXTLINE(misc-use-anonymous-namespace,readability-function-cognitive-complexity)
TEST_CASE("General function quadrature for plane 3D", "[impl]")
{
  using namespace qugar;
  using namespace qugar::impl;
  ...
  for (const auto normal : normals) {
    ...
    const auto plane = std::make_shared<funcs::Plane<3>>(origin, normal);
