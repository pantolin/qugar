...
TEST_CASE("General function quadrature for ellipse (2D)", "[impl]")
{
  ...
  const auto ellipse = std::make_shared<funcs::Ellipsoid<2>>(semi_axes, system);
...
// NOLINTNEXTLINE(misc-use-anonymous-namespace,readability-function-cognitive-complexity)
TEST_CASE("General function quadrature for ellipsoid (3D)", "[impl]")
{
  using namespace qugar;
  using namespace qugar::impl;
  ...
  const auto ellipsoid = std::make_shared<funcs::Ellipsoid<3>>(semi_axes, system);
